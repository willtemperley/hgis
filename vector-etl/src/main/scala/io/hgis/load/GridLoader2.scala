...
import com.esri.core.geometry.{Geometry, OperatorExportToWkb}
import com.vividsolutions.jts.geom.Envelope
import io.hgis.domain.GridNode
import io.hgis.hdomain.{AnalysisUnit, ConvertsGeometry, GriddedEntity, GriddedObjectDAO}
...
/**
 *
  * This loads via hTree instead of via spatial DB
 *
 ...
 */
abstract class GridLoader2[E <: AnalysisUnit](val clazz: Class[E], val geomType: Geometry.Type = Geometry.Type.Polygon) extends ConvertsGeometry with GridLoaderX {

  val file = new File("src/main/resources/gridcells.dat")
  val hTree = HTree(new FileInputStream(file))
  ...
  def executeLoad(table: HTableInterface) {
    ...
    for (analysisUnitId <- ids.zipWithIndex) {
      ...
      val gridCells = getHGrid(analysisUnit.geom)
  ...
  def getHGrid(env: Geometry): Array[TGridNode] = {

    val node = hTree.findSmallestContainingNode(env)
    hTree.findLeaves(node).toArray
...
//    q.getResultList.asInstanceOf[java.util.ArrayList[GridNode]]
  ...
  def executeGridding(analysisUnit: E, gridCells: Array[TGridNode], buffer: ListBuffer[Put] = new ListBuffer[Put]): ListBuffer[Put] = {
    ...
    val gridGeoms = gridCells.map(f => f.geom)
    val gridIds = gridCells.map(f => f.gridId)
